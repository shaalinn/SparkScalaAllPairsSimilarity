...
void l2apIndexRow(idx_t rid, params_t *params, da_csr_t *docs, ptr_t *endptr, da_invIdxJ_t *invIdx,
	...
	for ( endptr[rid] = rowptr[rid+1], j = rowptr[rid]; j < endptr[rid]; j++ ) {
		myval = rowval[j];
		b1 += (double) myval * gk_min(rwgts[rid], cwgts[rowind[j]]);
...
idx_t l2apProcessCandidates(params_t *params, idx_t rid, da_csr_t *docs, val_t *lengths, val_t *sums,
...
#ifdef DP1
		if ( cval + ((double) gk_min(rfiwgts[cid] * rsum,  rwgt * sums[endptr[cid]]) ) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP++;
			#endif
			continue;
...
#ifdef DP5
		if( cval + ((double) gk_min(endptr[cid]-rowptr[cid], sz) * rfiwgts[cid] * rwgt) < simT){
			#ifdef EXTRACOUNTS
				params->nPruneDotP++;
			#endif
			continue;
	...
	#ifdef DP2
		if ( cval + ((double) gk_min(rfiwgts[cid] * rsum,  hashwgt[rowind[j]] * sums[j]) ) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
	...
	#elif defined(DP3)
		if ( cval + ((double) gk_min(rfiwgts[cid] * hashsum[rowind[j]],  rwgt * sums[j]) ) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
	...
	#elif defined(DP4)
		if ( cval + ((double) gk_min(rfiwgts[cid] * hashsum[rowind[j]],  hashwgt[rowind[j]] * sums[j]) ) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
	...
	#elif defined(DP6)
		if ( cval + ((double) gk_min(p, sz) * rfiwgts[cid] * hashwgt[rowind[j]]) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
	...
	#elif defined(DP7)
		if ( cval + ((double) gk_min(p, hashsz[rowind[j]]) * rfiwgts[cid] * rwgt) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
	...
	#elif defined(DP8)
		if ( cval + ((double) gk_min(p, hashsz[rowind[j]]) * rfiwgts[cid] * hashwgt[rowind[j]]) < simT ){
			#ifdef EXTRACOUNTS
				params->nPruneDotP2++;
			#endif
			continue;
