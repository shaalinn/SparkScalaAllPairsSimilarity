...
//forward declaration
void da_csr_WriteClean(ptr_t **bptr, idx_t **bind, val_t **bval, char * filename, FILE *fpout);
...
void inline da_csr_WriteClean(ptr_t **bptr, idx_t **bind, val_t **bval, char * filename, FILE *fpout){
	if (sizeof(ptr_t) != sizeof(int32_t))
		gk_free((void**) bptr, LTERM);
...
/*! Writes the row-based structure of a matrix into a file.
    ...
    \param numbering is either 1 or 0 indicating if the internal 0-based
           ...
           is only applicable when DA_FMT_CSR or DA_FMT_CLUTO are used.

    Notes: - Formats DA_FMT_BINROW and DA_FMT_BINCOL assume 32 bit ints for indices and
           pointers and floats for values.
           - Writing out DA_FMT_BINCOL assumes the column based structure of the matrix
           exists. All other formats assume the row structure exists. An exception
           will be thrown if they do not.
...
void da_csr_Write(da_csr_t *mat, char *filename, char format, char writevals, char numbering)
{
	...
	nnz = (format == DA_FMT_BINCOL) ? mat->colptr[mat->ncols] : ptr[nr];
	if (sizeof(ptr_t) != sizeof(int32_t)){
		if(format == DA_FMT_BINCOL){
			if(!mat->colptr)
				gk_errexit(SIGERR, "DA_FMT_BINCOL and no mat->colptr!\n");
			bptr = da_pmalloc(mat->ncols+1, "da_csr_Write: bptr");
			for(i=0; i <= mat->ncols; i++)
				bptr[i] = mat->colptr[i];
		} else {
			bptr = da_pmalloc(mat->nrows+1, "da_csr_Write: bptr");
			for(i=0; i <= mat->nrows; i++)
				bptr[i] = ptr[i];
		}
	} else
		bptr = (format == DA_FMT_BINCOL) ? (ptr_t *)mat->colptr : (ptr_t *)ptr;
	if (sizeof(idx_t) != sizeof(int32_t)){
		bind = da_imalloc(nnz, "da_csr_Write: bind");
		if(format == DA_FMT_BINCOL){
			for(i=0; i < nnz; i++)
				bind[i] = mat->colind[i];
		} else {
	...
	if (format == DA_FMT_BINAP) {
		...
		fwrite(&nr, sizeof(idx_t), 1, fpout);

		for (i = 0; i < nr; i++) {
			len = bptr[i+1] - bptr[i];
			fwrite(&len, sizeof(idx_t), 1, fpout);
			if (len > 0) {
				fwrite(bind + bptr[i], sizeof(idx_t), len, fpout);
	...
	if (format == DA_FMT_BINAPB) {
		...
		for (i = 0, vId = 1; i < nr; i++, vId++) {
			fwrite(&vId, sizeof(idx_t), 1, fpout);
	...
	if (format == DA_FMT_BINROW) {
		...
		fwrite(&nr, sizeof(idx_t), 1, fpout);
		fwrite(&nc, sizeof(idx_t), 1, fpout);
		fwrite(bptr, sizeof(ptr_t), nr+1, fpout);
		fwrite(bind, sizeof(idx_t), nnz, fpout);
		if (writevals)
	...
	if (format == DA_FMT_BINCOL) {
		...
		fwrite(&nr, sizeof(idx_t), 1, fpout);
		fwrite(&nc, sizeof(idx_t), 1, fpout);
		fwrite(bptr, sizeof(ptr_t), nc + 1, fpout);
		fwrite(bind, sizeof(idx_t), nnz, fpout);
		if (writevals)
	...
	if (format == DA_FMT_BIJV) {
		...
		for (i = 0; i < nr; i++) {
			...
			for (j = bptr[i]; j < bptr[i+1]; j++) {
				edge[1] = bind[j];
				fwrite(edge, sizeof(ptr_t), 2, fpout);
				if (writevals)
	...
	if (format == DA_FMT_IJV) {
		...
		for (i = 0; i < mat->nrows; i++) {
			for (j = bptr[i]; j < bptr[i+1]; j++) {
				if (writevals)
					fprintf(fpout, "%zu "PRNT_IDXTYPE" %.8f\n", i + numbering,
							bind[j] + numbering, bval[j]);
				else
					fprintf(fpout, "%zu "PRNT_IDXTYPE"\n", i + numbering,
							bind[j] + numbering);
			}
		}
	...
	if (format == DA_FMT_CLUTO) {
		fprintf(fpout, PRNT_IDXTYPE" "PRNT_IDXTYPE" %zu\n",
				nr, nc, nnz);
		if (val)
			writevals = 1;
		numbering = 1;
	...
	for (i = 0; i < nr; i++) {
		for (j = bptr[i]; j < bptr[i+1]; j++) {
			fprintf(fpout, " "PRNT_IDXTYPE,
					bind[j] + (numbering ? 1 : 0));
			if (writevals)
				fprintf(fpout, " %g", bval[j]);
		}
		fprintf(fpout, "\n");
	}
