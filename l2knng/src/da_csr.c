...
da_csr_t** da_csr_Split(const da_csr_t* const mat, const idx_t* const color)
{
	...
	smats = (da_csr_t **)da_malloc(sizeof(da_csr_t *)*ncolors, "da_csr_Split: smats");
	for (i=0; i<ncolors; ++i) {
		...
		const char format, char writevals, char numbering)
{
	...
	if (format == DA_FMT_BINAP) {
		...
		if (!bval)
			da_errexit( "da_csr_Write: Format requires values but rowval not present.\n");
	...
	if (format == DA_FMT_SMAT) {
		...
		if (!mat->rowptr)
			da_errexit("da_csr_Write: Row data needed for MV_FMT_SMAT format SMAT file.");
 ...
 */
/**************************************************************************/
da_csr_t* da_csr_HighAvgFilter(const da_csr_t* mat, const char what, const float percent)
{
	ssize_t i, j, k, n, nnz;
	idx_t nrows, ncols, ncand, maxlen=0;
	ptr_t *rowptr, *nrowptr;
	...
	if(!mat->rowptr)
		da_errexit( "da_csr_HighAvgFilter: row structure needed for this function.");
	...
	nrowind = nmat->rowind = da_imalloc(rowptr[nrows], "da_csr_HighAvgFilter: nrowind");
	nrowval = nmat->rowval = da_vmalloc(rowptr[nrows], "da_csr_HighAvgFilter: nrowval");

	switch(what){
	case DA_COL:
		nrowptr = nmat->rowptr = da_pmalloc(nrows+1, "da_csr_HighAvgFilter: nrowptr");
		...
		nmat->rowind = da_irealloc(nmat->rowind, nnz, "da_csr_HighAvgFilter: nrowind");
		nmat->rowval = da_vrealloc(nmat->rowval, nnz, "da_csr_HighAvgFilter: nrowval");
		break;
	case DA_ROW:
		nmat->nrows -= n;
		nrowptr = nmat->rowptr = da_pmalloc(nmat->nrows+1, "da_csr_HighAvgFilter: nrowptr");
...
double* da_csr_ComputeMeans(const da_csr_t* const mat, const char what)
{
	...
	if(ptr){
		...
		for (i=0; i<n; ++i){
			means[i] = da_vsum(ptr[i+1]-ptr[i], val+ptr[i], 1) / (double)(ptr[i+1]-ptr[i]);
		}
...
void da_csr_ComputeSquaredNorms(da_csr_t* const mat, const char what)
{
	...
	case DA_ROW:
		...
		if (mat->rnorms) da_free((void **)&mat->rnorms, LTERM);
	...
	case DA_COL:
		...
		if (mat->cnorms) da_free((void **)&mat->cnorms, LTERM);
