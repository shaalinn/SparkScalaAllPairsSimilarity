...
\warning 
The implementation provided here uses the \c da_ prefix for all variables
...
static char *da_getopt_initialize (int, char **, char *);
static int da_getopt_internal(int argc, char **argv, char *optstring,
		struct da_option *longopts, int *longind, int long_only);
...
`--' can cause `getopt' to return -1 with `da_optind' != ARGC.
 */
/*************************************************************************/
static enum
{
...
The other is elements [last_nonopt,da_optind), which contains all
...
static void exchange (char **argv)
{
	...
	first_nonopt += (da_optind - last_nonopt);
	last_nonopt = da_optind;
}
 ...
 */
/*************************************************************************/
static char *da_getopt_initialize (int argc, char **argv, char *optstring)
{
	/* Start processing options with ARGV-element 1 (since ARGV-element 0
     is the program name); the sequence of previously skipped
     non-option ARGV-elements is empty.  */

	first_nonopt = last_nonopt = da_optind;
...
/*
   ...
   updating `da_optind' and `nextchar' so that the next call to `getopt' can
   resume the scan with the following option character or ARGV-element.

   If there are no more option characters, `getopt' returns -1.
   Then `da_optind' is the index in ARGV of the first ARGV-element
   that is not an option.  (The ARGV-elements have been permuted
   so that those that are not options now come last.)

   OPTSTRING is a string containing the legitimate option characters.
   If an option character is seen that is not listed in OPTSTRING,
   return '?' after printing an error message.  If you set `da_opterr' to
   zero, the error message is suppressed but we still return '?'.

   If a char in OPTSTRING is followed by a colon, that means it wants an arg,
   so the following text in the same ARGV-element, or the text of the following
   ARGV-element, is returned in `da_optarg'.  Two colons mean an option that
   wants an optional arg; if there is text in the current ARGV-element,
   it is returned in `da_optarg', otherwise `da_optarg' is set to zero.
   ...
   LONGOPTS is a vector of `struct da_option' terminated by an
		...
		struct da_option *longopts, int *longind, int long_only)
{
	...
	if (da_optind == 0 || !da_getopt_initialized) {
		if (da_optind == 0)
			da_optind = 1;	/* Don't scan ARGV[0], the program name.  */
		optstring = da_getopt_initialize (argc, argv, optstring);
		da_getopt_initialized = 1;
	}

	/* Test whether ARGV[da_optind] points to a non-option argument.
     Either it does not have option syntax, or there is an environment flag
     from the shell indicating it is not an option.  The later information
     is only used when the used in the GNU libc.  */
# define NONOPTION_P (argv[da_optind][0] != '-' || argv[da_optind][1] == '\0')
	...
	if (nextchar == NULL || *nextchar == '\0') {
       ...
       moved back by the user (who may also have changed the arguments).  */
		if (last_nonopt > da_optind)
			last_nonopt = da_optind;
		if (first_nonopt > da_optind)
			first_nonopt = da_optind;

		if (ordering == PERMUTE) {
			/* If we have just processed some options following some non-options,
	 exchange them so that the options come first.  */

			if (first_nonopt != last_nonopt && last_nonopt != da_optind)
				exchange ((char **) argv);
			else if (last_nonopt != da_optind)
				first_nonopt = da_optind;

			/* Skip any additional non-options
	 and extend the range of non-options previously skipped.  */

			while (da_optind < argc && NONOPTION_P)
				da_optind++;
       ...
       then skip everything else like a non-option.  */
		...
		if (da_optind != argc && !strcmp (argv[da_optind], "--")) {
			da_optind++;
	 ...
	 that we previously skipped, so the caller will digest them.  */
       ...
       Skip the initial punctuation.  */
		...
		nextchar = (argv[da_optind] + 1 + (longopts != NULL && argv[da_optind][1] == '-'));
	}
	...
	if (longopts != NULL && (argv[da_optind][1] == '-' || (long_only && (argv[da_optind][2] || !strchr(optstring, argv[da_optind][1]))))) {
		char *nameend;
		struct da_option *p;
		struct da_option *pfound = NULL;
		int exact = 0;
		int ambig = 0;
		int indfound = -1;
		int option_index;
		...
		if (pfound != NULL) {
			...
			if (*nameend) {
				...
				if (pfound->has_arg)
					...
					if (print_errors) {
						if (argv[da_optind - 1][1] == '-')
							/* --option */
							fprintf(stderr, "%s: option `--%s' doesn't allow an argument\n", argv[0], pfound->name);
						else
							/* +option or -option */
							fprintf(stderr, "%s: option `%c%s' doesn't allow an argument\n", argv[0], argv[da_optind - 1][0], pfound->name);
					}

					nextchar += strlen (nextchar);

					da_optopt = pfound->val;
					return '?';
				}
			}
       ...
       or the option starts with '--' or is not a valid short
		...
		if (!long_only || argv[da_optind][1] == '-' || strchr(optstring, *nextchar) == NULL) {
			if (print_errors) {
				if (argv[da_optind][1] == '-')
					/* --option */
					...
					/* +option or -option */
					fprintf(stderr, "%s: unrecognized option `%c%s'\n", argv[0], argv[da_optind][0], nextchar);
			}
			nextchar = (char *) "";
			da_optind++;
			da_optopt = 0;
	...
	/* Look at and handle the next short option-character.  */
	{
		...
		/* Increment `da_optind' when we start to process its last character.  */
		if (*nextchar == '\0')
			++da_optind;
	   ...
	   we must advance to the next element now.  */
				...
				/* We already incremented `da_optind' once; increment it again when taking next ARGV-elt as argument.  */
				da_optarg = argv[da_optind++];

			/* da_optarg is now the argument, see if it's in the table of longopts.  */

			for (nextchar = nameend = da_optarg; *nameend && *nameend != '='; nameend++)
				/* Do nothing.  */ ;
...
The da_getopt() function gets the next option argument from the argument
...
#da_optind against the \c argc parameter to check this.

\return  
If the option has an argument, da_getopt() returns the argument by storing
it in the variable #da_optarg. You don't ordinarily need to copy the
#da_optarg string, since it is a pointer into the original \c argv array,
not into a static area that might be overwritten.

\return  
If da_getopt() finds an option character in \c argv that was not included
in options, or a missing option argument, it returns `?' and sets the 
external variable #da_optopt to the actual option character.
If the first character of options is a colon (`:'), then da_getopt()
returns `:' instead of `?' to indicate a missing option argument. 
In addition, if the external variable #da_opterr is nonzero (which is
the default), da_getopt() prints an error message.  This variable is
set by da_getopt() to point at the value of the option argument,
for those options that accept arguments.


da_getopt() has three ways to deal with options that follow non-options
 ...
 */
/*************************************************************************/
int da_getopt(int argc, char **argv, char *options)
{
	return da_getopt_internal(argc, argv, options, NULL, NULL, 0);
}
...
\param long_options describes the long options to accept. See the 
       defintion of ::da_option for more information.
\param opt_index this is a returned variable.  For any long option, 
       da_getopt_long() tells you the index in the array \c long_options
...
\return
When da_getopt_long() encounters a short option, it does the same thing
that da_getopt() would do: it returns the character code for the option,
and stores the options argument (if it has one) in #da_optarg.

\return
When da_getopt_long() encounters a long option, it takes actions based
on the flag and val fields of the definition of that option.

\return
If flag is a null pointer, then da_getopt_long() returns the contents
...
can decode these values after da_getopt_long() returns. If the long
...
In this case, da_getopt_long() returns 0.

\return
When a long option has an argument, da_getopt_long() puts the argument
value in the variable #da_optarg before returning. When the option has
no argument, the value in #da_optarg is a null pointer. This is
how you can tell whether an optional argument was supplied.

\return
When da_getopt_long() has no more options to handle, it returns -1,
and leaves in the variable #da_optind the index in argv of the next
remaining argument. 
 */
/*************************************************************************/
int da_getopt_long( int argc, char **argv, char *options,
		struct da_option *long_options, int *opt_index)
{
	return da_getopt_internal (argc, argv, options, long_options, opt_index, 0);
}
...
Like da_getopt_long(), but '-' as well as '--' can indicate a long option.
If an option that starts with '-' (not '--') doesn't match a long option,
but does match a short option, it is parsed as a short option instead.  
 */
/*************************************************************************/
int da_getopt_long_only(int argc, char **argv, char *options,
		struct da_option *long_options, int *opt_index)
{
	return da_getopt_internal(argc, argv, options, long_options, opt_index, 1);
}
