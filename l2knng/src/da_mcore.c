...
da_mcore_t* da_mcoreCreate(const size_t coresize)
{
	...
	mcore = (da_mcore_t *)da_malloc(sizeof(da_mcore_t), "da_mcoreCreate: mcore");
	memset(mcore, 0, sizeof(da_mcore_t));

	mcore->coresize = coresize;
	mcore->corecpos = 0;

	mcore->core = (coresize == 0 ? NULL : da_malloc(mcore->coresize, "da_mcoreCreate: core"));

	/* allocate the memory for keeping track of malloc ops */
	mcore->nmops = 2048;
	mcore->cmop  = 0;
	mcore->mops  = (da_mop_t *)da_malloc(mcore->nmops*sizeof(da_mop_t), "da_mcoreCreate: mcore->mops");
...
da_mcore_t* da_gkmcoreCreate(void)
{
	...
	mcore->nmops = 2048;
	mcore->cmop  = 0;
	if ((mcore->mops = (da_mop_t *)malloc(mcore->nmops*sizeof(da_mop_t))) == NULL) {
		free(mcore);
		return NULL;
	}
	...
	while (mcore->cmop > 0) {
		...
		case DA_MOPT_CORE: /* core free */
			if (mcore->corecpos < mcore->mops[mcore->cmop].nbytes)
				da_errexit("Internal Error: wspace's core is about to be over-freed [%zu, %zu, %zd]\n",
...
void da_mcoreAdd(da_mcore_t* const mcore, const int type, const size_t nbytes, void* const ptr)
{
	if (mcore->cmop == mcore->nmops) {
		...
		if (mcore->mops == NULL)
			da_errexit("MEMORY ERROR: ***Memory allocation for gkmcore failed.\n");
	}
