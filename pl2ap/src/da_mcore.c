...
/*************************************************************************/
da_mcore_t *
da_mcoreCreate (const size_t coresize)
{
  da_mcore_t *mcore;

  mcore =
    (da_mcore_t *) da_malloc (sizeof (da_mcore_t), "da_mcoreCreate: mcore");
  memset (mcore, 0, sizeof (da_mcore_t));
  ...
  mcore->core =
    (coresize ==
     0 ? NULL : da_malloc (mcore->coresize, "da_mcoreCreate: core"));

  /* allocate the memory for keeping track of malloc ops */
  mcore->nmops = 2048;
  mcore->cmop = 0;
  mcore->mops =
    (da_mop_t *) da_malloc (mcore->nmops * sizeof (da_mop_t),
			    "da_mcoreCreate: mcore->mops");
...
/*************************************************************************/
da_mcore_t *
da_gkmcoreCreate (void)
{
  da_mcore_t *mcore;

  if ((mcore = (da_mcore_t *) malloc (sizeof (da_mcore_t))) == NULL)
    return NULL;
  memset (mcore, 0, sizeof (da_mcore_t));
  ...
  mcore->cmop = 0;
  if ((mcore->mops =
       (da_mop_t *) malloc (mcore->nmops * sizeof (da_mop_t))) == NULL)
    {
      free (mcore);
      return NULL;
    }
...
/*************************************************************************/
void
da_mcoreDestroy (da_mcore_t ** r_mcore, const int showstats)
{
  da_mcore_t *mcore = *r_mcore;
  ...
  da_free ((void **) &mcore->core, &mcore->mops, &mcore, LTERM);
...
/*************************************************************************/
void
da_gkmcoreDestroy (da_mcore_t ** r_mcore, const int showstats)
{
  da_mcore_t *mcore = *r_mcore;
  ...
  if (showstats)
    printf ("\n da_mcore statistics\n"
	    ...
	    mcore->num_hallocs,
	    mcore->size_hallocs, mcore->cur_hallocs, mcore->max_hallocs);
...
/*************************************************************************/
void *
da_mcoreMalloc (da_mcore_t * const mcore, size_t nbytes)
{
  void *ptr;
  ...
  if (mcore->corecpos + nbytes < mcore->coresize)
    {
      ...
      da_mcoreAdd (mcore, DA_MOPT_CORE, nbytes, ptr);
    }
  else
    {
      /* service this request from the heap */
      ptr = da_malloc (nbytes, "da_mcoremalloc: ptr");

      da_mcoreAdd (mcore, DA_MOPT_HEAP, nbytes, ptr);
    }
...
/*************************************************************************/
void
da_mcorePush (da_mcore_t * const mcore)
{
  da_mcoreAdd (mcore, DA_MOPT_MARK, 0, NULL);
  /* printf("MCPPUSH:   %zu\n", mcore->cmop-1); */
}
...
/*************************************************************************/
void
da_gkmcorePush (da_mcore_t * const mcore)
{
  da_gkmcoreAdd (mcore, DA_MOPT_MARK, 0, NULL);
  /* printf("MCPPUSH:   %zu\n", mcore->cmop-1); */
}
...
/*************************************************************************/
void
da_mcorePop (da_mcore_t * const mcore)
{
  while (mcore->cmop > 0)
    {
      mcore->cmop--;
	...
	{
	case DA_MOPT_MARK:	/* push marker */
	  goto DONE;
	  break;

	case DA_MOPT_CORE:	/* core free */
	...
	case DA_MOPT_HEAP:	/* heap free */
	  da_free ((void **) &mcore->mops[mcore->cmop].ptr, LTERM);
	  mcore->cur_hallocs -= mcore->mops[mcore->cmop].nbytes;
	  break;

	default:
	  da_errexit ("MEMORY ERROR: Unknown mop type of %d\n",
...
/*! This function frees all mops since the last push. This version is
    for poping the gkmcore and it uses free instead of da_free.
 */
/*************************************************************************/
void
da_gkmcorePop (da_mcore_t * const mcore)
{
  while (mcore->cmop > 0)
    {
      mcore->cmop--;
...
/*************************************************************************/
void
da_mcoreAdd (da_mcore_t * const mcore, const int type, const size_t nbytes,
	     void *const ptr)
{
  if (mcore->cmop == mcore->nmops)
    {
      mcore->nmops *= 2;
      mcore->mops = realloc (mcore->mops, mcore->nmops * sizeof (da_mop_t));
      if (mcore->mops == NULL)
	da_errexit
	  ("MEMORY ERROR: ***Memory allocation for gkmcore failed.\n");
    }
  ...
  switch (type)
	...
	mcore->max_hallocs = mcore->cur_hallocs;
      break;
    default:
      da_errexit ("MEMORY ERROR: Incorrect mcore type operation.\n");
    }
}
...
/*************************************************************************/
void
da_gkmcoreAdd (da_mcore_t * const mcore, const int type, const size_t nbytes,
	       void *const ptr)
{
  if (mcore->cmop == mcore->nmops)
    {
...
/*************************************************************************/
void
da_mcoreDel (da_mcore_t * const mcore, void *const ptr)
{
  int i;

  for (i = mcore->cmop - 1; i >= 0; i--)
    {
      if (mcore->mops[i].type == DA_MOPT_MARK)
	da_errexit ("MEMORY ERROR: Could not find pointer %p in mcore\n",
		    ptr);

      if (mcore->mops[i].ptr == ptr)
	{
	  if (mcore->mops[i].type != DA_MOPT_HEAP)
	    da_errexit ("MEMORY ERROR: Trying to delete a non-HEAP mop.\n");
  ...
  da_errexit ("MEMORY ERROR: mcoreDel should never have been here!\n");
}
...
/*************************************************************************/
void
da_gkmcoreDel (da_mcore_t * const mcore, void *const ptr)
{
  int i;
  ...
  da_errexit ("MEMORY ERROR: gkmcoreDel should never have been here!\n");
}
