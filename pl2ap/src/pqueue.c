...
DA_MKPQUEUE (da_ivmq, da_ivpq_t, da_ivkv_t, idx_t, val_t, da_ivkvmalloc,
	     -FLT_MAX, val_gt)
#undef val_gt
#define val_lt(a, b) ((a) < (b))
/** NOTE: Inserts are not checked in da_ivpq. Check item does not exist before inserting */
  DA_MKPQUEUE (da_ivpq, da_ivpq_t, da_ivkv_t, idx_t, val_t, da_ivkvmalloc,
	     FLT_MAX, val_lt)
/****************************************************************************/
...
/**************************************************************************/
da_iapq_t *
da_iapqCreateShared (size_t maxsize, size_t maxnodes, ptr_t * locator)
{
  da_iapq_t *queue;
...
/**************************************************************************/
void
da_iapqInit (da_iapq_t * queue, size_t maxsize, size_t maxnodes,
	     ptr_t * locator)
{
  queue->nnodes = 0;
  ...
  queue->heap = da_iakvsmalloc (maxsize, (da_iakv_t)
				{
				-1, 0.0}, "da_PQInit: heap");
  if (locator)
...
/**************************************************************************/
void
da_iapqInitLocator (da_iapq_t * queue)
{
  ptr_t i;
  size_t nnodes;
  ptr_t *locator;
...
/**************************************************************************/
void
da_iapqResetLocator (da_iapq_t * queue)
{
  ptr_t i;
  size_t nnodes;
  ptr_t *locator;
...
/**************************************************************************/
void
da_iapqDestroy (da_iapq_t * queue)
{
  if (queue == NULL)
    return;
  da_iapqFree (queue);
...
/***************************************************************************/
int
da_iapqInsert (da_iapq_t * queue, idx_t node, accum_t val)
{
  ptr_t i, j;
  ptr_t *locator = queue->locator;
  da_iakv_t *heap = queue->heap;
...
/***************************************************************************/
int
da_iapqInsertHeap (da_iapq_t * queue, idx_t node, val_t val)
{
  ptr_t i, j, loc;
  da_iakv_t *heap = queue->heap;
...
/**************************************************************************/
void
da_iapqUpdate (da_iapq_t * queue, idx_t node, accum_t newval)
{
  ptr_t i, j;
  size_t nnodes;
  val_t oldval;
...
/**************************************************************************/
void
da_iapqUpdateHeap (da_iapq_t * queue, ptr_t loc, idx_t node, val_t newval)
{
  ptr_t i, j;
  size_t nnodes;
  val_t oldval;
  ...
  ASSERT (loc != -1) oldval = heap[loc].val;
  if (oldval == newval)
    return;
  i = loc;
...
/**************************************************************************/
int
da_iapqDelete (da_iapq_t * queue, idx_t node)
{
  ptr_t i, j;
  size_t nnodes;
  val_t newval, oldval;
  ...
  if ((i = queue->nnodes) > 0)
      ...
      while ((j = 2 * i + 1) < queue->nnodes)
	{
	  if (val_lt (heap[j].val, val))
	    {
	      if (j + 1 < queue->nnodes
		  && val_lt (heap[j + 1].val, heap[j].val))
		j = j + 1;
	      heap[i] = heap[j];
	      locator[heap[i].key] = i;
	      i = j;
...
/*************************************************************************/
da_iakv_t
da_iapqGetTopHeap (da_iapq_t * queue)
{
  ptr_t i, j;
  da_iakv_t *heap;
  idx_t node;
...
/**************************************************************************/
idx_t
da_iapqSeeTopKey (da_iapq_t * queue)
{
...
/**************************************************************************/
accum_t
da_iapqSeeTopVal (da_iapq_t * queue)
{
...
/**************************************************************************/
accum_t
da_iapqSeeVal (da_iapq_t * queue, idx_t node)
{
  ptr_t *locator;
  da_iakv_t *heap;
...
/**************************************************************************/
accum_t
da_iapqSeeValHeap (da_iapq_t * queue, idx_t node)
{
  ptr_t i, j, loc;
  da_iakv_t *heap = queue->heap;
...
/**************************************************************************/
int
da_iapqExists (da_iapq_t * queue, idx_t node)
{
  ptr_t *locator;
...
/**************************************************************************/
int
da_iapqExistsHeap (da_iapq_t * queue, idx_t node)
{
  ptr_t i, j, loc;
  da_iakv_t *heap = queue->heap;
...
/**************************************************************************/
int
da_iapqCheckHeap (da_iapq_t * queue)
{
  ptr_t i, j;
  size_t nnodes;
  ptr_t *locator;
...
void
da_iapqPrintAll (da_iapq_t * queue)
{
  size_t i;
