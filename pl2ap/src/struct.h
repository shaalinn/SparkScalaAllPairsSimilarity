 ...
 */
#ifndef _COSGRAPH_STRUCT_H_
#define _COSGRAPH_STRUCT_H_
...
typedef struct da_sims_t {
    ...
    ptr_t * ptr;
        ...
        /**
 * A set of values to hash
 */
typedef struct l2ap_hash_t {
    val_t val; /** value for feature cid */
    val_t len; /** suffix L2-norm (length) starting at feature cid */
    val_t pmax; /** prefix max value up and including feature cid */
} l2ap_hash_t;
...
typedef struct da_tsearch_t {
    ...
    idx_t * cands;
    idx_t * nbrids;
    da_sim_t * nbrslst;    // neighbors in list form
    da_csr_t * nbrs;
    ...
    ssize_t nCacheHit;     // number of cache hits
    ssize_t nCacheMiss;    // number of cache misses
    ssize_t nmask;         // number of rows processed with mask hash
    accum_t percAccum;     // sum of percent of vector pair traversed prior to pruning
...
typedef struct {
    ...
    float eps;              /* Similarity threshold */
    int32_t k;              /* k for kNN Search */
    double eps2;            /* Similarity threshold squared */
    ...
    char symout;            /* Whether to add higher neighbors - symmetric output */
    ...
    size_t ninnz;           /* Number of non-zeros in each inv index block */
    ...
    char * dFile;           /* The filestem of the database data CSR matrix file (for NN Search). */
    ...
    ssize_t nCacheHit;     // number of cache hits
    ssize_t nCacheMiss;    // number of cache misses
    val_t percAccum;       // sum of percent of vector pair traversed prior to pruning
...
typedef struct {
    ...
    val_t * idxlens; // array of lengths associated with vals
    idx_t * cands;   // candidates array
    val_t * accum;   // accumulator for a given doc
} da_sfind_t;
