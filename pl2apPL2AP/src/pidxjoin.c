...
void da_ompijFindNeighbors (params_t * params, da_csr_t * mat,
...
void pijFindNeighbors (params_t * params)
{
    ...
    if (params->fpout && params->fmtWrite != DA_FMT_CSR
            && params->fmtWrite != DA_FMT_IJV)
        da_errexit ("Mode pij can only output data in CSR or IJV formats.");
...
//    }
    ...
    da_AllocMatrix ((void ***) &cand, sizeof (da_ivkv_t), params->nthreads,
            params->ndrows);
    marker =
            da_iAllocMatrix (params->nthreads, params->ndrows, -1,
    ...
    for (qID = 0; qID < nrows; qID += params->nqrows) {
        ...
        for (dID = 0; dID < nrows; dID += params->ndrows) {
            ...
            da_ompijFindNeighbors (params, docs, pmat, qID, nqrows, dID, ndrows,
            ...
            if (params->fmtWrite == DA_FMT_CSR)
                for (i = 0; i < nqrows; i++) {
                    for (j = 0; j < nallhits[i]; j++) {
                        if (qID + i > allhits[i][j].key || params->symout)
...
/*! Computes the neighbors of a set of rows against the documents in
    pmat using OpenMP */
/**************************************************************************/
void da_ompijFindNeighbors (params_t * params, da_csr_t * mat, da_csr_t * pmat,
        int qID, int nqrows,
        int dID, int ndrows,
...
#pragma omp for private(j, k, l, nhits, nnbrs, newhits, oldhits, noldhits) schedule(dynamic, NITERS)
        for (i = 0; i < nqrows; i++) {
            ...
            nhits = da_ApssGetSimilarRows (pmat, qID - dID + i, 1,
                    mat->rowptr[qID + i + 1] -
                    mat->rowptr[qID + i],
